...
if __name__ == "__main__":
    ...
    for delta in delta_list:
        ...
        for _ in range(num_dataset):
            ...
            # mask
            mask = np.logical_and(t_grid_hat[0] > 0.2, t_grid_hat[0] < 0.8)

            p_val0 = test_from_delta(Delta[mask], rho)
            # get the end time
            et = time.time()
        ...
        data_file = os.path.join(save_path, f'p_val_one_split_oracle_mask_delta_{delta}_at_inf_ratio_{inf_ratio}_rho_{rho}_num_dataset_{num_dataset}.txt')
        np.savetxt(data_file, p_val)
        print('p-values saved to', save_path)
        time_file = os.path.join(save_path, f'run_time_one_split_oracle_mask_delta_{delta}_at_inf_ratio_{inf_ratio}_rho_{rho}_num_dataset_{num_dataset}.txt')
    ...
    for _ in range(len(delta_list)):
        delta = delta_list[_]
        try:
            p_val = pd.read_csv(save_path + f'/p_val_one_split_oracle_mask_delta_{delta}_at_inf_ratio_{inf_ratio}_rho_{rho}_num_dataset_{num_dataset}.txt', header = None)
            run_time = pd.read_csv(save_path + f'/run_time_one_split_oracle_mask_delta_{delta}_at_inf_ratio_{inf_ratio}_rho_{rho}_num_dataset_{num_dataset}.txt', header = None)
    ...
    data_file = os.path.join(save_path, f'rej_rate_one_split_oracle_mask_at_inf_ratio_{inf_ratio}_rho_{rho}_num_dataset_{num_dataset}.txt')

...
if __name__ == "__main__":
    ...
    inf_ratio = 0.2 #0.15 #0.3
    rho = 0.05 #0.115 #0.135 #0.15 #0.4
    ...
    for delta in delta_list:
        ...
        data_file = os.path.join(save_path, f'p_val_one_split_oracle_delta_{delta}_at_inf_ratio_{inf_ratio}_rho_{rho}_num_dataset_{num_dataset}.txt')
        np.savetxt(data_file, p_val)
        print('p-values saved to', save_path)
        time_file = os.path.join(save_path, f'run_time_one_split_oracle_delta_{delta}_at_inf_ratio_{inf_ratio}_rho_{rho}_num_dataset_{num_dataset}.txt')
    ...
    for _ in range(len(delta_list)):
        delta = delta_list[_]
        try:
            p_val = pd.read_csv(save_path + f'/p_val_one_split_oracle_delta_{delta}_at_inf_ratio_{inf_ratio}_rho_{rho}_num_dataset_{num_dataset}.txt', header = None)
            run_time = pd.read_csv(save_path + f'/run_time_one_split_oracle_delta_{delta}_at_inf_ratio_{inf_ratio}_rho_{rho}_num_dataset_{num_dataset}.txt', header = None)
    ...
    data_file = os.path.join(save_path, f'rej_rate_one_split_oracle_at_inf_ratio_{inf_ratio}_rho_{rho}_num_dataset_{num_dataset}.txt')
